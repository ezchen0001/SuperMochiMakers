...
func to_screen_space(vec:Vector2):
	return vec * Vector2(1,-1) * Vector2(DisplayServer.window_get_size())

func pos_average (hand_data:Array):
	var sum = Vector2()
	for lm_id in range(NUM_LANDMARKS):
		var lm_data = hand_data[lm_id]
		var pos_cam = Vector3(lm_data[0], lm_data[1], lm_data[2]) - Vector3.ONE * 0.5
		var pos_xyz = Vector2(-pos_cam[0], -pos_cam[1])#, pos_cam[2]) * HAND_SCALE
		#pos_xyz.z += 12
		#_update_hand_landmark(lm_id, pos_xyz)
		sum += pos_xyz
	return to_screen_space(sum / NUM_LANDMARKS)

func reposition_node_landmarks(joints:Array, hand_data:Array):
	for lm_id in range(NUM_LANDMARKS):
		var lm_data = hand_data[lm_id]
		var pos_cam = Vector3(lm_data[0], lm_data[1], lm_data[2]) - Vector3.ONE * 0.5
		var pos_xyz = Vector2(-pos_cam[0], -pos_cam[1])
		joints[lm_id].position = to_screen_space(pos_xyz)
